...
class IrActionsReport(models.Model):
    ...
    def _render_onlyoffice_pdf_prepare_streams(self, report_ref, data, res_ids=None):
        ...
        if res_ids:
            ...
            # The report record itself is read with sudo() above, but the records being
            # rendered must not be: the Document Server fetches their field values back
            # through the fill callback, so skipping this check would turn printing into a
            # way to read records the user cannot otherwise see. Callers that legitimately
            # render for someone else (a queue job, a cron) are expected to run under that
            # user's env rather than to bypass the check here.
        ...
        if is_onlyoffice_needed:
            ...
            for res_id in all_res_ids_wo_stream:
                # A failure here used to be swallowed with a warning, which left the
                # record's stream empty and silently dropped that document from the
                # output — the reader has no way to tell a missing page from a page that
                # was never meant to be there. Fail loudly and name the record instead.
                try:
